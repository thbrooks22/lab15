...
(* to_float s -- Converts a stream s of ints to the corresponding
   floats. *)
...
(* within -- Return the index and the value of the first element in
   the stream s to be within epsilon of its following element. *)
...
(* Here, we compute pi to three decimal places and print out the
   result. Any more digits than this will take a long time. See the rest
   of the lab for speeding up the process. *)
